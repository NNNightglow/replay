...
class ChinaTradingCalendar:
    ...
    def __init__(self):
        ...
        self.china_holidays = holidays.China(years=range(2000, 2031))
        # 中国股市特殊非交易日（手动维护）
    ...
    def get_non_trading_days_in_month(self, year: int, month: int) -> List[dict]:
        """
        ...
        non_trading_days = self.get_non_trading_days_in_range(start_date, end_date)
        
        result = []
        for day in non_trading_days:
            info = self.get_holiday_info(day)
            if info['is_special_non_trading']:
                day_type = 'special'
                day_name = '特殊休市'
            elif info['is_holiday']:
                day_type = 'holiday'
                day_name = info['holiday_name'] or '法定节假日'
            else:
                day_type = 'weekend'
                day_name = '周末'
            result.append({
